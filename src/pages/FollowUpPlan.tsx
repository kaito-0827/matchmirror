...
export default function FollowUpPlan() {
  ...
  // reportId未指定時、候補者一覧の先頭を自動選択。候補者がいなければ旧localStorageフローにフォールバック
  useEffect(() => {
    if (reportId) return
    ...
    if (candidatesLoaded) {
      const stored = localStorage.getItem('mm_report_id')
      if (stored) {
        setSearchParams({ reportId: stored, label: '候補者' }, { replace: true })
      }
    }
  ...
  useEffect(() => {
    const generate = async () => {
      if (!reportId) {
        if (candidatesLoaded && candidates.length === 0 && !localStorage.getItem('mm_report_id')) {
      ...
      try {
        ...
        if (reportId === localStorage.getItem('mm_report_id')) {
          const raw = localStorage.getItem('mm_report')
          if (raw) {
            try {
              const report = JSON.parse(raw)
              if (report.gaps?.length > 0) {
                report.gaps.forEach((g: { axis: string; title: string }, i: number) => {
                  if (fallbackTasks[i]) fallbackTasks[i].axis = g.axis
                })
              }
            } catch { /* ignore */ }
          }
        ...
        {!loading && !error && (
          <>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <Button variant="secondary" onClick={() => navigate('/company/dashboard')} style={{ padding: '12px 28px' }}>
                ← ダッシュボードへ
              </Button>
            </div>
          </>
