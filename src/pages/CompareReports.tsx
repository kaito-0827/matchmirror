...
export default function CompareReports() {
  ...
  const openDetail = (it: CompareItem) => {
    ...
    localStorage.removeItem('mm_report') // その社のフルレポートを生成し直す
      ...
      <div style={{ padding: '32px 48px', maxWidth: 1100 }}>
        ...
        {loading ? (
          ...
          <Card style={{ padding: 24 }}>
            ...
            <Button onClick={() => navigate('/candidate')} variant="secondary">← 診断開始へ</Button>
          </Card>
        ) : (
        ...
        {!loading && !error && (
          <div style={{ marginTop: 24, display: 'flex', gap: 12 }}>
            <Button variant="secondary" onClick={() => navigate('/candidate/matches')} style={{ padding: '10px 22px' }}>
              他のおすすめ企業も見る →
            </Button>
            <Button variant="secondary" onClick={() => navigate('/candidate')} style={{ padding: '10px 22px' }}>
              ← 診断開始へ
            </Button>
          </div>
        )}
